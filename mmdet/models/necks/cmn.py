...
class SpMiddleFHD(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        points_mean = torch.zeros_like(voxel_features[:,:4])
        ...
        x, point_misc, points_mean,pointwise = self.backbone(x, points_mean, is_test=False)
        x = x.dense()
        N, C, D, H, W = x.shape
        x = x.view(N, C * D, H, W)
        x = self.fcn(x)
        return x, point_misc, points_mean,pointwise
...
class VxNet(nn.Module):
    ...
    def forward(self, x, points_mean, is_test):
        ...
        return out, (points_mean, point_cls, point_reg), points_mean,pointwise
