...
from mmdet.ops.iou3d.iou3d_utils import boxes3d_to_corners3d_lidar_torch,\
    boxes_iou3d_gpu
...
class SSDRotateHead(nn.Module):
    ...
    def loss(self, box_preds, cls_preds, dir_cls_preds, gt_bboxes, gt_labels, anchors, anchors_mask, cfg):
        ...
        labels, targets, assigned_gts, ious = multi_apply(create_target_torch,
                                            anchors, gt_bboxes,
        ...
        assigned_gts = torch.stack(assigned_gts)
        ...
        positive_inds = labels.view(-1)>0
        box_preds = box_preds.view(box_preds.shape[0],-1,self._box_code_size)
        box_preds = second_box_decode(box_preds, anchors)
        box_preds = box_preds.view(-1,self._box_code_size)[positive_inds]
        assigned_gts = assigned_gts.view(-1,self._box_code_size)[positive_inds]
        # corners_loss
        box_preds_corners = boxes3d_to_corners3d_lidar_torch(box_preds)
        corners_target = boxes3d_to_corners3d_lidar_torch(assigned_gts)
        assigned_gts[...,6] = assigned_gts[...,6] + np.pi
        corners_target_flip = boxes3d_to_corners3d_lidar_torch(assigned_gts)
        corners_loss = huber_loss(torch.min(
            torch.norm(box_preds_corners - corners_target, dim=1), \
            torch.norm(box_preds_corners - corners_target_flip, dim=1)
        ), delta=1.0)
        
        corners_loss_reduced = corners_loss / batch_size
        corners_loss_reduced *= 2
        ...
        return dict(rpn_loc_loss=loc_loss_reduced, \
                    rpn_cls_loss=cls_loss_reduced, \
                    rpn_dir_loss=dir_loss_reduced,
                    rpn_corners_loss=corners_loss_reduced    
                    )
    ...
    def get_guided_anchors(self, box_preds, cls_preds, dir_cls_preds, anchors, anchors_mask, gt_bboxes, thr=.1):
        ...
        for box_preds, cls_preds, dir_preds, a_mask, gt_boxes in zip(
            ...
            new_scores.append(cls_preds[selected])
        return new_boxes,new_scores

    def rcnn_get_guided_anchors(self, box_preds, cls_preds, anchors, anchors_mask, gt_bboxes, thr=.1):
        batch_size = box_preds.shape[0]
        anchors = torch.cat(anchors,0).unsqueeze(0)
        batch_box_preds = box_preds.view(batch_size, -1, self._box_code_size)
        batch_cls_preds = cls_preds.view(batch_size, -1)
        batch_box_preds = second_box_decode(batch_box_preds, anchors)
        ...
        for box_preds, cls_preds, gt_boxes in zip(
                batch_box_preds, batch_cls_preds, gt_bboxes
        ):
            ...
            new_boxes.append(box_preds)
            bbox_score.append(cls_preds[selected])
        return new_boxes, bbox_score
...
class PSWarpHead(nn.Module):
    def __init__(self, grid_offsets, featmap_stride, in_channels, num_class=1, window_size=[4,7]):
        super(PSWarpHead, self).__init__()
        self._num_class = num_class
        out_channels = window_size[0] * window_size[1]

        self.gen_grid_fn = partial(gen_sample_grid, window_size=window_size, grid_offsets=grid_offsets, spatial_scale=1 / featmap_stride)
    ...
    def forward(self, x, guided_anchors, is_test=False):
        ...
        for i, ga in enumerate(guided_anchors):
            if len(ga) == 0:
                ...
                continue
            (xs, ys) = self.gen_grid_fn(ga[:, [0, 1, 3, 4, 6]])#xywlr
            im = x[i]
            out = bilinear_interpolate_torch_gridsample(im, xs, ys)
            pixel_wise_features.append(out.squeeze(1).squeeze(-1).permute(1,0))
        if is_test:
            return bbox_scores, pixel_wise_features
        else:
            return torch.cat(pixel_wise_features, 0)
    ...
    def loss(self, box_preds, cls_preds, gt_bboxes, gt_labels, anchors, anchors_mask, cfg):
        ...
        targets = torch.cat(targets,0).unsqueeze(0)
        anchors = torch.cat(anchors,0).unsqueeze(0)
        assigned_gts = torch.cat(assigned_gts,0).unsqueeze(0)
        cls_weights, reg_weights, cared = self.prepare_loss_weights(labels)
        ...
        return dict(rcnn_loc_loss=loc_loss_reduced,
                    rcnn_cls_loss=cls_loss_reduced, 
                    )
