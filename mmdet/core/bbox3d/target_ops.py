...
def create_target_torch(all_anchors,
    ...
    assigned_gts = torch.zeros(
        (num_inside, box_code_size), dtype=all_anchors.dtype, device=gt_boxes.device)
    if len(gt_boxes) > 0 and anchors.shape[0] > 0:
        bbox_targets[fg_inds, :] = box_encoding_fn(
            gt_boxes[anchor_to_gt_argmax[fg_inds], :], anchors[fg_inds, :])
        assigned_gts[fg_inds, :] = gt_boxes[anchor_to_gt_argmax[fg_inds], :]
    ...
    if anchor_mask is not None:
        ...
        assigned_gts = _unmap(assigned_gts, total_anchors, anchor_mask, fill=0)

    return (labels, bbox_targets, assigned_gts, anchor_to_gt_max)
